// ----------------------------------------------------
// Created by Roguey 5/3/2016 (roguey.co.uk)
...
function draw_galaxy()
{
	...
	// x cord (map_items)
	var map_item_min_x = Math.abs( Math.min.apply(Math, map_items_x) );
	var map_item_max_x = Math.max.apply(Math, map_items_x);

	// z cord (map_items)
	var map_item_min_z = Math.abs( Math.min.apply(Math, map_items_z) );
	...
	for (i=0; i<map_items_x.length; i++)
	{
		...
		// cords
		var pos_x = Math.round( c_mid_x + (map_items_x[i] / grid_size) );
		var pos_y = Math.round( c_mid_y - (map_items_z[i] / grid_size) );
	...
	for (var i=0; i< (gate_connection.length/2); i++)
	{
		...
		if ( (search1_id > 0) || (search2_id > 0))
		{
			...
			var start_x = Math.round( c_mid_x + (map_items_x[search1_id] / grid_size));
			var start_y = Math.round( c_mid_y - (map_items_z[search1_id] / grid_size));

			
			var end_x = Math.round( c_mid_x + (map_items_x[search2_id] / grid_size));
			var end_y = Math.round( c_mid_y - (map_items_z[search2_id] / grid_size));
			...
			ctx.beginPath();
				...
				var arrowhead_1_angle	= angle_line - 30;
				var arrowhead_2_angle	= angle_line + 30;

				var arrowhead_1_cords	= move_angle(start_x, start_y, arrowhead_1_angle, -16);
				var arrowhead_2_cords	= move_angle(start_x, start_y, arrowhead_2_angle, -16);
			...
			ctx.beginPath();
				...
				var arrowhead_1_cords	= move_angle(end_x, end_y, arrowhead_1_angle, 16);
				var arrowhead_2_cords	= move_angle(end_x, end_y, arrowhead_2_angle, 16);
